    ...
    public class FileProviderMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            if (context.Request.Query.ContainsKey("dir-meta"))
            {
                ...
                var dirDescriptor = 
                    new HttpDirectoryContentsDescriptor(dirContents, CreatePhysicalPathResolver(context, true));
        ...
        private Func<string, string> CreatePhysicalPathResolver(HttpContext context, bool isDirRequest)
        {
            ...
            return isDirRequest ? (Func<string, string>)(name => $"{schema}://{host}:{port}{pathBase}{path}/{name}") :
